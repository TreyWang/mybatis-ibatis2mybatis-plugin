	...
	<xsl:template match="isNotEmpty">
		<xsl:element name="if">
			<xsl:attribute name="test">
				...
				<xsl:text> @com.bibenet.middleware.utils.ValidateUtils@isNotEmpty(</xsl:text><xsl:value-of select="@property" /><xsl:text>)</xsl:text>
			</xsl:attribute>
			<xsl:value-of select="@prepend" />
			<xsl:apply-templates/>
		</xsl:element>
	...
	<xsl:template match="dynamic">
		<xsl:choose>
			<xsl:when test="@prepend">
				<xsl:choose>
					<xsl:when test="@prepend = 'set'">
						<xsl:element name="trim">
							<xsl:attribute name="prefix"><xsl:value-of select="@prepend" /></xsl:attribute>
							<xsl:attribute name="prefixOverrides"><xsl:text>,</xsl:text></xsl:attribute>
							<xsl:apply-templates/>
						</xsl:element>
					</xsl:when>
					<xsl:when test="@prepend = 'where'">
	...
	<xsl:template match="isEmpty">
		<xsl:element name="if">
			<xsl:attribute name="test">
				...
				<xsl:text> @com.bibenet.middleware.utils.ValidateUtils@isEmpty(</xsl:text><xsl:value-of select="@property" /><xsl:text>)</xsl:text>
			</xsl:attribute>
			<xsl:value-of select="@prepend" />
			<xsl:apply-templates/>
		</xsl:element>
	...
	<xsl:template match="iterate">
		...
		<xsl:element name="foreach">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:attribute name="collection"><xsl:text>list</xsl:text></xsl:attribute>
				</xsl:otherwise>
			</xsl:choose>
